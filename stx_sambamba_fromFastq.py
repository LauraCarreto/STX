...
def main():
    ...
    #CHANGED:
    dResult['matches'] = run_pileup_analysis(ref, bowtie2_output_dir, sNGSSampleID)
    #print 'MatchedStxs', aMatchedStxs

    #NEW: generate a coverage summary file for alignments with all references using sambamba
    ...
    #NEW: create box plots of coverage for alignments with stx references
...
def run_pileup_analysis(ref, bowtie2_output_dir, s_name):
    '''
...
def parse_pileup(mapped_bases, reference):
    ...
    for each in mapped_bases:
        ...
        if each in uniq_pos:
            ...
            if depth > 10:
                if uniq_mapping / float(depth) >= 0.4:
                    i += 1
                    #print each, mapped_bases[each]
